...
public interface SpringDataPetRepository extends PetRepository, Repository<Pet, Integer>, PetRepositoryOverride {
    ...
    @Override
    @Query(
        value = "SELECT DISTINCT pet FROM Pet pet " +
            "LEFT JOIN FETCH pet.type " +
            "LEFT JOIN FETCH pet.owner " +
            "WHERE (:name IS NULL OR pet.name LIKE CONCAT(:name, '%')) " +
            "AND (:typeName IS NULL OR pet.type.name = :typeName) " +
            "AND (:ownerId IS NULL OR pet.owner.id = :ownerId)",
        countQuery = "SELECT COUNT(pet) FROM Pet pet " +
            "WHERE (:name IS NULL OR pet.name LIKE CONCAT(:name, '%')) " +
            "AND (:typeName IS NULL OR pet.type.name = :typeName) " +
            "AND (:ownerId IS NULL OR pet.owner.id = :ownerId)")
    Page<Pet> findPets(@Param("name") String name, @Param("typeName") String typeName,
